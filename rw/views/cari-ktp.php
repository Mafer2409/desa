    ...
    <div class="container">
        ...
        <table id="bootstrap-data-table" class="table table-striped table-sm">
            ...
            <tbody>
                <?php
                if ($bln == 0) {
                    $sql = mysqli_query($con, "SELECT * FROM ktp, user, rt WHERE user.user_rt_id = rt.rt_id AND rt.rt_rw_id = '$idrw' AND ktp.ktp_user = user.user_id AND YEAR(ktp.ktp_tanggal_verifikasi) = '$thn' GROUP BY user.user_id");
                } else {
                    $sql = mysqli_query($con, "SELECT * FROM ktp, user, rt WHERE user.user_rt_id = rt.rt_id AND rt.rt_rw_id = '$idrw' AND ktp.ktp_user = user.user_id AND YEAR(ktp.ktp_tanggal_verifikasi) = '$thn' AND MONTH(ktp.ktp_tanggal_verifikasi) = '$bln' GROUP BY user.user_id");
                }
                $no = 1;
