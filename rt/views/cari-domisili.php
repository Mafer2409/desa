        ...
        <div class="card-header">
            <form class="mx-3" action="" method="post">
                <div class="row">
                    ...
                    <div class="col-lg-2">
                        <div class="row">
                            <a href="../assets/report/report-rt/report-domisili-rt.php?bln=<?= $bln ?>&thn=<?= $thn ?>&idrt=<?= $idrt ?>" class="btn btn-primary ml-2 mt-5" target="_blank">Cetak</a>
                        </div>
                    </div>
                </div>
            </form>
...
<div class="container-xxl py-6">
    <div class="container">
        <h3 class="mb-2">Data Administrasi domisili RT: <?= $namart ?></h3>
        <table id="bootstrap-data-table" class="table table-striped table-sm">
            ...
            <tbody>
                <?php
                if ($bln == 0) {
                    $sql = mysqli_query($con, "SELECT * FROM domisili, user WHERE domisili.domisili_user = user.user_id AND YEAR(domisili.domisili_tanggal_verifikasi) = '$thn' AND domisili.domisili_rt = '$idrt'");
                } else {
                    $sql = mysqli_query($con, "SELECT * FROM domisili, user WHERE domisili.domisili_user = user.user_id AND YEAR(domisili.domisili_tanggal_verifikasi) = '$thn' AND MONTH(domisili.domisili_tanggal_verifikasi) = '$bln' AND domisili.domisili_rt = '$idrt'");
                }
                $no = 1;
