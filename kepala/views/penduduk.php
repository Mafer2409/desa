<div class="content-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h3>Data Penduduk</h3>
            <a href="#" class="btn btn-md btn-success mb-2" data-toggle="modal" data-target="#ModalTambahPenduduk"><i class="fas fa-plus"></i> Tambah Warga</a>
            <div class="card">
                ...
                <div class="card-body table-responsive">
                    <table id="bootstrap-data-table" class="table table-striped table-sm">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $sql = mysqli_query($con, "SELECT * FROM penduduk, rt WHERE penduduk.penduduk_rt_id = rt.rt_id");
                            while ($data = mysqli_fetch_assoc($sql)) {
                            ?>
                                <tr>
                                    <td><?= $no++; ?>.</td>
                                    <td><?= $data['penduduk_nama']; ?></td>
                                    <td><?= $data['penduduk_tempat_lahir']; ?>, <?= $data['penduduk_tgl_lahir']; ?></td>
                                    <td><?= $data['rt']; ?></td>
                                    <td><?= $data['penduduk_jk']; ?></td>
                                    <td><?= $data['penduduk_pekerjaan']; ?></td>
                                    <td><?= $data['penduduk_status_tinggal']; ?></td>
                                    <td>
                                        <a href="?page=info_penduduk&id=<?= $data['penduduk_id'] ?>" class="text-primary"><i class="fas fa-info-circle"></i></a>
                                        <!-- <a href="?page=edit_penduduk&id=<?= $data['penduduk_id'] ?>" class="text-info"><i class="fas fa-edit"></i></a>
                                        <a href="?page=hapus_penduduk&id=<?= $data['penduduk_id'] ?>" onclick="return confirm('Yakin ingin menghapus data ini ???')" class="text-danger"><i class="fas fa-trash"></i></a> -->
                                    </td>
                                </tr>
                            <?php
                            }
...
<!-- Modal -->
<div class="modal fade" id="ModalTambahPenduduk" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
if (isset($_POST['daftar'])) {
    ...
    $sql = mysqli_query($con, "INSERT INTO penduduk VALUES('', '$user_nik', '$user_nama', '$user_tempat_lahir', '$user_tanggal_lahir', '$user_rt_id', '$user_agama', '$user_jk', '$user_wn', '$user_alamat', '$user_status_tinggal', '$user_pekerjaan', '$user_status_kawin', '$user_jenjang_pendidikan')");

    if ($sql) {
        echo "<script>alert('Pendaftaran Berhasil !');window.location='?page=penduduk';</script>";
    } else {
        echo "<script>alert('Pendaftaran Gagal ! Ulangi Pendaftaran !');window.location='?page=penduduk';</script>";
    }
}
?>