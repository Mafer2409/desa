...
if ($bln == '0' && $thn == '0') {
    $html .= '<center><h3 style="font-family:sans-serif; margin-bottom: 10px; margin-top: -25px;">LAPORAN DATA KK</h3></center>';
} elseif ($bln == '0' && $thn != '0') {
    $html .= '<center><h3 style="font-family:sans-serif; margin-bottom: 10px; margin-top: -25px;">LAPORAN DATA KK TAHUN ' . $thn . '</h3></center>';
} else {
    $html .= '<center><h3 style="font-family:sans-serif; margin-bottom: 10px; margin-top: -25px;">LAPORAN DATA KK BULAN ' . $bulan . ' TAHUN ' . $thn . '</h3></center>';
}

if ($bln == '0' && $thn == '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kk WHERE user.user_rt_id = rt.rt_id AND user.user_id = kk.kk_user");
} elseif ($bln == '0' && $thn != '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kk WHERE user.user_rt_id = rt.rt_id AND user.user_id = kk.kk_user AND YEAR(kk.kk_tanggal_verifikasi) = '$thn'");
} else {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kk WHERE user.user_rt_id = rt.rt_id AND user.user_id = kk.kk_user AND MONTH(kk.kk_tanggal_verifikasi) = '$bln' AND YEAR(kk.kk_tanggal_verifikasi) = '$thn'");
}
