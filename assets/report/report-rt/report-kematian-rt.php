...
if ($bln == '0' && $thn == '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kematian WHERE kematian.kematian_rt = rt.rt_id AND user.user_id = kematian.kematian_user AND kematian.kematian_rt = '$idrt'");
} elseif ($bln == '0' && $thn != '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kematian WHERE kematian.kematian_rt = rt.rt_id AND user.user_id = kematian.kematian_user AND YEAR(kematian.kematian_tanggal_verifikasi) = '$thn' AND kematian.kematian_rt = '$idrt'");
} else {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, kematian WHERE kematian.kematian_rt = rt.rt_id AND user.user_id = kematian.kematian_user AND MONTH(kematian.kematian_tanggal_verifikasi) = '$bln' AND YEAR(kematian.kematian_tanggal_verifikasi) = '$thn' AND kematian.kematian_rt = '$idrt'");
}
