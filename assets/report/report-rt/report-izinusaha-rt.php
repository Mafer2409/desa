...
if ($bln == '0' && $thn == '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, izinusaha WHERE user.user_rt_id = rt.rt_id AND izinusaha.izinusaha_rt = '$idrt' AND user.user_id = izinusaha.izinusaha_user");
} elseif ($bln == '0' && $thn != '0') {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, izinusaha WHERE user.user_rt_id = rt.rt_id AND izinusaha.izinusaha_rt = '$idrt' AND user.user_id = izinusaha.izinusaha_user AND YEAR(izinusaha.izinusaha_tanggal_verifikasi) = '$thn'");
} else {
    $sql = mysqli_query($con, "SELECT * FROM user, rt, izinusaha WHERE user.user_rt_id = rt.rt_id AND izinusaha.izinusaha_rt = '$idrt' AND user.user_id = izinusaha.izinusaha_user AND MONTH(izinusaha.izinusaha_tanggal_verifikasi) = '$bln' AND YEAR(izinusaha.izinusaha_tanggal_verifikasi) = '$thn'");
}
...
while ($data = mysqli_fetch_array($sql)) {
 ...
 <td>" . $data['izinusaha_nama_usaha'] . "</td>
 <td>" . $data['izinusaha_jenis_usaha'] . "</td>
 <td>" . $data['izinusaha_nama_pemilik'] . "</td>
 <td>" . $data['izinusaha_nik_pemilik'] . "</td>
 <td>" . $data['izinusaha_status'] . "</td>
