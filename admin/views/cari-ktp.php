                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <div class="row">
                                    ...
                                    <a href="../assets/report/report-admin/report-ktp.php?bln=<?= $bln ?>&thn=<?= $thn ?>" class="btn btn-primary ml-2 mt-5" target="_blank">Cetak</a>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                <div class="card-body table-responsive">
                    <table id="bootstrap-data-table" class="table table-striped table-sm">
                        ...
                        <tbody>
                            <?php
                            if ($bln == '0') {
                                $sql = mysqli_query($con, "SELECT * FROM ktp, user, rt, rw WHERE rt.rt_rw_id = rw.rw_id AND ktp.ktp_user = user.user_id AND ktp.ktp_rt = rt.rt_id AND YEAR(ktp.ktp_tanggal_verifikasi) = '$thn'");
                            } else {
                                $sql = mysqli_query($con, "SELECT * FROM ktp, user, rt, rw WHERE rt.rt_rw_id = rw.rw_id AND ktp.ktp_user = user.user_id AND ktp.ktp_rt = rt.rt_id AND MONTH(ktp.ktp_tanggal_verifikasi) = '$bln' AND YEAR(ktp.ktp_tanggal_verifikasi) = '$thn'");
                            }
                            $no = 1;
...
if (isset($_POST['cari'])) {
    ...
    if ($bln == '') {
        echo "<script>window.location='?page=cari-ktp&bln=0&thn=$thn';</script>";
    } else {
        echo "<script>window.location='?page=cari-ktp&bln=$bln&thn=$thn';</script>";
    }
}
